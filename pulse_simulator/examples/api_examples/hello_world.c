 ...
 *
 * Description:
 * This app uses timers to print "Hello World" down the screen on a 3 second
 * internal. Also, button down and up events are printed to the screen.
 *
 *
...
#define TIMEOUT_VAL (3000)

// Forward Declaration
void print_hello_world(int data);

// This function is called once after the watch has booted up
// and the OS has loaded
void main_app_init()
{
    pulse_register_timer(TIMEOUT_VAL, &print_hello_world, 1);
}

void print_hello_world(int data)
{
    printf("Hello World %d\n", data);
    pulse_register_timer(TIMEOUT_VAL, &print_hello_world, ++data);
}
